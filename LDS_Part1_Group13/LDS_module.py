...
def convert(line):
    ...
    otherwise they remain the original values
    """
    output = []
    for e in line:
        try:
...
def extract(fileIn, attribute, attributesIndex, sur_Key=True):
    ...
    """
    columns = [attributesIndex[e] for e in attribute]
...
def addGeoDimension(dicIn, idx_countrycode):
    """
    Add continent and country_name (switched with country_code)
    """
    for key, value in dicIn.items():
        country_code = value[idx_countrycode]
        
        # Given country_code add continent to the tuple 
        try:
...
def cleaner(dictIn, idx_da):   
    """
    cleaner() function discards the hours and keeps only the date
    """
...
def addDescription(dict_1, dict_2): # dict_1=temp_subject, dict_2=dict_subject_metadata
    ...
    for key, value in dict_1_new.items():
        ...
        dictOut[str(value)] = ", ".join([x[0] for x in description])
